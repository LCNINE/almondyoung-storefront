...
export function CustomThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (savedTheme && ["light", "black-friday", "valentine", "christmas"].includes(savedTheme)) {
        setTheme(savedTheme)
      }
    }
  }, [])
  ...
  useEffect(() => {
    ...
    // 기존 테마 클래스 제거
    document.body.classList.remove("light", "black-friday", "valentine", "christmas")
    
    // 새 테마 클래스 추가
    document.body.classList.add(theme)
    
    // HTML 요소에도 클래스 추가 (CSS 변수 적용을 위해)
    document.documentElement.classList.remove("light", "black-friday", "valentine", "christmas")
