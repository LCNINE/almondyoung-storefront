...
const PIM_API_BASE_URL =
  process.env.NEXT_PUBLIC_PIM_API_BASE_URL

// API 요청 타임아웃 설정 (15초 - 상품 데이터는 더 클 수 있음)
...
export async function fetchCategories(): Promise<PimCategory[]> {
  ...
  const res = await fetch(`${PIM_API_BASE_URL}/categories`, {
    method: "GET",
    headers: {
  ...
  if (!res.ok) {
    console.error('❌ [fetchCategories] API 요청 실패:', res.status, res.statusText)
 ...
 */
//카드형
export async function getAllProductList(params?: {
  page?: number
  limit?: number
  status?: string
  brand?: string
  ...
  stock?: string[]
  tags?: string[]
}, signal?: AbortSignal): Promise<PimListResponse<PimProductDetail>> {
  ...
  const url = `${PIM_API_BASE_URL}/masters?${searchParams.toString()}`
  ...
  try {
    ...
    if (!res.ok) {
      if (res.status >= 500) throw new Error("서버에 일시적인 문제가 발생했습니다. 잠시 후 다시 시도해주세요.")
      if (res.status >= 400) throw new Error("요청을 처리할 수 없습니다. 입력 정보를 확인해주세요.")
    ...
    const total = json?.total ?? json?.data?.total ?? json?.count ?? items.length
...
//상세정보포함
export async function getPimProductDetail(id: string): Promise<PimProductDetail> {
  
  const url = `${PIM_API_BASE_URL}/masters/${id}`
  ...
  if (!contentType?.includes("application/json")) {
    console.error('❌ [getPimProductDetail] JSON이 아닌 응답:', text.substring(0, 200))
...
export async function getPimCategoryProducts(
  categoryId: string,
  params?: { page?: number; limit?: number; sort?: string; brand?: string; search?: string; tags?: string[]; stock?: string[] }
): Promise<PimListResponse<PimProductDetail>> {
  console.log('🌐 [getPimCategoryProducts] API 호출 시작:', { categoryId, params })
  
  if (!PIM_API_BASE_URL) throw new Error("PIM API URL이 설정되지 않았습니다.")
  ...
  const url = `${PIM_API_BASE_URL}/masters?${categoryId}` 
